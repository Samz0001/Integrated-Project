...
def fetch_youtube_videos(query, max_results=25):
    ...
    for item in data.get("items", []):
        ...
        videos.append({"videoId": video_id, "title": title, "thumbnail": thumbnail})
    return videos
...
def login():
    ...
    return jsonify({"token": access_token, "username": username, "message": "Login successful!"}), 200
...
def get_history():
    current_user = get_jwt_identity()
    user = users_collection.find_one({"username": current_user}, {"history": 1, "_id": 0})
...
def share_mood():
    ...
    share_text = (
        ...
        f"Try Moodmusic at https://moodmusic.xo"
    )
